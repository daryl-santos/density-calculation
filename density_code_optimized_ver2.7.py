...
### Get data
galaxies = pd.read_csv('457183_HSC_galaxies_photz_greaterthan_0_redshiftrange_correct_csvfile.csv', skip_blank_lines=True)            
edgepoints = pd.read_csv('edge_0.0025_withoutholes.csv', skip_blank_lines=True)            # Please fill the right filename
...
gal = list(zip(id_gal, ra_gal, dec_gal, galaxies['redshift'])) # redshift values of galaxies should be named as 'redshift'
...
def edge_calculate(galpd, edgepd):
    ...
    galpd['edge_dist'] = final_dist # in radians
    galpd['edge_dist_arcmin'] = final_dist*(60*180)/np.pi # in arcmin
    return galpd
...
def distance_calculate(galpd):
    ...
    for i, row in galpd.iterrows():
        ...
        galpd.loc[i, 'tenth_dist_uncorrected'] = tenth_dist_uncorrected[i]
        
### PART 3: Edge Correction
### Select galaxies near edge
        flag[i] = np.where(galpd['edge_dist'][i] < tenth_dist_uncorrected[i], 1, 0)
...
### First, identify the value of n, which corresponds to the index of the true "10th nearest neighbor"
        if flag[i] == 1:
            c = 2*np.sqrt((tenth_dist_uncorrected[i])**2 - (galpd['edge_dist'][i])**2) # c = length of chord
...
### Find index and then the value of 10th smallest distance
    ...
    galpd['flag'] = flag
    galpd['tenth_dist_corrected'] = tenth_dist_val_new # in radians
...
galpd['density_uncorrected'] = 10/(galpd['tenth_dist_uncorrected']**2)
galpd['tenth_dist_uncorrected_arcmin'] = galpd['tenth_dist_uncorrected']*(60*180)/math.pi
galpd['density_uncorrected_arcmin'] = 10/(galpd['tenth_dist_uncorrected_arcmin']**2)

galpd['tenth_dist_corrected_arcmin'] = galpd['tenth_dist_corrected']*(60*180)/math.pi
galpd['density_corrected'] = 10/(galpd['tenth_dist_corrected']**2)
galpd['density_corrected_arcmin'] = 10/(galpd['tenth_dist_corrected_arcmin']**2)

galpd.drop(['ra_cos', 'ra_sin', 'dec_cos', 'dec_sin'], axis=1, inplace=True)
galpd.to_csv('457183_HSC_galaxies_arcmin_corrected.csv', index=False)
print('Done density!')
